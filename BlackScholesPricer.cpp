...
double BlackScholesPricer::operator()() const {
    ...
    if (_option_vanilla != nullptr) {
        ...
        if ((*_option_vanilla)._type == optionType::Call)
        ...
        if ((*_option_digital)._type == optionType::Call) {
...
double BlackScholesPricer::delta() const {
    if (_option_vanilla != nullptr) {
        double d1 = (1 / (_volatility * sqrt(_option_vanilla->getExpiry()) * (log(_asset_price / (*_option_vanilla)._strike) + (_interest_rate + pow(_volatility, 2) / 2) * _option_vanilla->getExpiry())));
        if ((*_option_vanilla)._type == optionType::Call)
            return std::erfc(d1);
        else
